...
# Create entry widget
entry = tk.Entry(root, width=20, font=('Arial', 16), bg='pink', fg='white')
...
for (text, row, col) in buttons:
    btn = tk.Button(root, text=text, width=5, height=2, font=('Arial', 14), bg='white' if text in ['=', '.','<-','√','π','AC'] else 'gray' if text.isdigit() else 'black', fg='black' if text in ['=', '.','<-','√','π','AC'] else 'white')
    if text == 'AC':
